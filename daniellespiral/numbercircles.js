class NumberCircle{
	constructor(num,cx,cy,radius,primes){
        ...
        if(this.length % 2 == 0){
        	current = current - Math.PI / this.length;
        }
		var shift = 2*Math.PI / (this.length);
		if(factors[this.length-1] > 7){
		...
		if(this.length>1){
			for(var i = 0; i < this.length; i++){
				...
				if(factors[i] == 1){
					context.fillStyle = "#0E2A38";
				}
				else if(factors[i] == 2){
					context.fillStyle = "#3399ff";
				}
				else if(factors[i] == 3){
					context.fillStyle = "#F2D06B";
				}
				else if(factors[i] == 5){
					context.fillStyle = "#F28749";
				}
				else if(factors[i] == 7){
					context.fillStyle = "#265902";
				}
				else{
					context.fillStyle = "#D26363";
				}
				...
				if(conditionsArray.indexOf(factors[i]) == -1){
					...
					var text = context.fillText(""+factors[i], this.x + (13*radius/16) * Math.cos(((current+next)/2)), this.y + (13*radius/16) * Math.sin(((current+next)/2)));
				}
				current = next;
				next = (next + shift);
			}
