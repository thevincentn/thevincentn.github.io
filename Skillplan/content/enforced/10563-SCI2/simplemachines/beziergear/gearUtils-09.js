...
(function()
{
  ...
  createGearOutline = function(module, teeth, pressureAngle, shaftRadius)
  {
    ...
    for (i=1; i<Z; i++)
    {
      rotToothData = rotateTooth(toothData, 2*Math.PI*i/teeth); // rotate the tooth to its position
      gearData.push(...rotToothData.slice(3));  // skip the initial "M",x,y command to make path continuos
    }
